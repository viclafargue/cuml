...
def test_predict_and_score(dataset, datatype, n_neighbors,
    ...
    if datatype == 'dask_array':
        ...
        hm_distributed_score = float(r2_score(np_y_test, y_pred))
        hm_distributed_score = round(hm_distributed_score, 3)
        assert hm_distributed_score == pytest.approx(handmade_local_score,
                                                     abs=1e-2)
